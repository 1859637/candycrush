...
public class Grid {
	...
	/**
	 * Uses the randomColor helper method to generate a Candy from the available
...
//        }
		for (int i = 0; i < candies.length; i ++) {
			if (candies[i] == null || candies.length == 0) {
				return new int[] {0};
			}
		}
        ...
        int currentStartIndex 	= 0; //temporary variable for tracking current starting index
	...
	public boolean isAdjacent(int row1, int col1, int row2, int col2) {
		...
		if ((row1 + 1 == row2 || row1 - 1 == row2) && (col1 == col2)) {
			return true;
		}else if((col1 + 1 == col2 || col1 - 1 == col2) && (row1 == row2)){
			return true;
		}else{
			return false;
		}
	...
	public void swapSpaces(int r1, int c1, int r2, int c2) {
		...
		Candy temp1 = grid[r1][c1];
		Candy temp2 = grid[r2][c2];
		grid[r1][c1] = temp2;
		grid[r2][c2] = temp1;
	...
	*/
	public int inARow() {
		...
		//use the findMaximum method that you wrote to get the max sequence of the row
